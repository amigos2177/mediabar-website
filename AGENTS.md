...
- **August 12 service-page CTR copy:** Rewrote titles, metas, H1s, hero decks, selected H2s, and FAQs on `/video-production/events`, `/live-streaming`, `/post-production`, `/corporate`, and `/commercials` to match high-impression GSC queries (event video production, live streaming, post-production, corporate video production San Antonio). Added Media Bar to the post-production title. Internal links now include `/locations/san-antonio`, `/pricing`, and relevant `/work/watch/*` examples. No invented claims. Brand/homepage left alone.
