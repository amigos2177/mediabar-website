...
- **September 26 Upper Deck preview:** Existing Superman watch page expanded with four selected videos from the supplied project spreadsheet: Michael Jordan 25th, Wayne Gretzky 40th, Marvel Ages, Legendary James Bond. Uses verified Vimeo metadata and user-confirmed 50-plus relationship copy. Click-to-load posters plus direct Vimeo fallback; desktop/mobile layout and image loading checked. Embedded playback did not render in the in-app browser and remains unverified. User explicitly approved publication; live release verification follows in the task.
...
- **September 26 approved homepage refinement:** Hero now reads "A clear plan from the start." CTAs precede unchanged proof badges, with brief guidance and compact mobile spacing. Ported the approved local preview onto current main without reverting the external stylesheet or recent content. Webpack production build (80 routes), TypeScript, targeted ESLint, and diff checks pass. User approved publication; release verification recorded in the task.
- **September 23 search-growth proof batch (publication approved; deployment pending):** Corporate page now links to ST Engineering, Sanctuary Wealth, and RBFCU project evidence; three watch pages have project-specific notes based on existing public descriptions and runtime metadata. Event example copy identifies the NAFA day-two recap. Watch VideoObject names use curated public titles; affected sitemap dates reflect this edit without altering upload dates. Production webpack build (80 routes), lint, structured-data suite, generated HTML assertions, desktop layout and 390px mobile cards/navigation pass. Authority follow-up: GSC reports 365 external links across 38 sites, concentrated on homepage; BBB contact details differ from the main site and sanantoniocorporatevideo.com is parked. No listings, DNS, outreach, indexing requests, or production deployment changed.
- **September 21 animated patient education (approved for publication; deployment pending):** Added a dedicated service page with the approved eye-exam concept art, click-to-play Vimeo demo (1229044258), production process, clinical review, quote-based scope, FAQs, metadata, and Service/Breadcrumb/FAQ schema. Added discovery links in navigation, footer, service index, medical and motion-graphics pages, and sitemap. Package pricing remains unapproved and is not displayed. SEO refinement: descriptive service H1 and local metadata, visible service definition, DM Sans typography, contextual headings, llms.txt listing, and keyword ownership mapping. Static HTML checks confirm FAQ/schema agreement, indexable metadata, sitemap inclusion, and discovery links; the structured-data suite now covers this service (13 service pages). Validation: lint, TypeScript and the 80-page webpack production build pass; existing structured-data checks pass. New route verified at 1440/820/390px with HTTP 200, one H1, correct canonical and schema types, working FAQ and contact navigation, no overflow, broken rendered images, or page errors.
- **September 2 Claude-audit follow-up (local branch; not published):** Added `alternateName: "Media Bar"` to the LocalBusiness entity, with a structured-data regression check. Repositioned `/faq` as the company-specific pricing, process, ownership, delivery, studio, and logistics FAQ while clarifying `/resources/video-production-faq` as the broader educational planning guide; added reciprocal navigation and truthful review dates. Strengthened the existing Texas TV commercial cost answer with production-vs-airtime metadata, an additional budget-scope FAQ, and a pricing-page link without inventing rates or creating a competing article. Updated affected sitemap dates. The Google Business Profile public entity URL was not added because it could not be verified reliably; profile alignment remains a human check. ESLint, TypeScript, `git diff --check`, the 77-route webpack production build, generated HTML checks, and structured-data verification pass. Link earning and the 6-8 week GSC review remain off-site/human follow-ups; no outreach was sent.
- **September 2 remaining SEO audit refinements (review ready; not published):** Tightened the San Antonio location title and four watch-page descriptions to search-result-friendly lengths using only public Media Bar service and process language. Corrected every color-contrast failure from the mobile `/video-production` Lighthouse audit, including small labels, red text accents, portfolio labels, and location-link text. Local production Lighthouse now reports accessibility 100 with zero contrast failures (up from 96), performance 95, LCP 2.9 s, and CLS 0. ESLint, TypeScript, `git diff --check`, the 77-route production build, structured-data verification, and generated metadata checks pass. Merge/publish remain human gates.
- **September 2 homepage critical CSS fix (live via PR #16):** Moved the homepage's large inline stylesheet into a scoped route stylesheet while preserving the branded typography and layout. Production-style mobile testing scored 96 performance with 2.7 s LCP and 63 ms H1 render delay; desktop scored 100 performance with 0.7 s LCP. PR #16 merged as `25b7444`; Vercel production deployment completed successfully, and the canonical homepage serves the external stylesheet with a substantially smaller HTML response.
- **August 31 Events CTR pass (review ready; not published):** `/video-production/events` only. Rewrote title, meta, Open Graph/Twitter (same `buildMetadata` source), and H1 to be more click-worthy at ~position 23. Title is now `San Antonio Event Video | Recaps & Sessions | Media Bar` (55). Meta leads with a San Antonio / Texas crew and recap/session deliverables (155). H1 is `Event Video Production For Conferences, Recaps, and Sessions.` Service JSON-LD description, sitemap, redirects, and body copy besides the H1 are unchanged. Merge/publish and GSC URL Inspection remain human gates.
- **August 29 Media Bar-only transcript and AI SEO pass (review ready; not published):** Left all client watch-page transcripts and client dialogue out of the site. Strengthened the seven first-party Media Bar Answers pages by adding their existing visible direct answers, takeaways, and Media Bar scripts to matching `Article.articleBody` structured data; connected each Article to its topic section, YouTube source, and the Media Bar Answers collection; and added all seven first-party answer URLs to `llms.txt`. Structured-data verification now requires those Article relationships while continuing to require zero `VideoObject` nodes on answer pages. ESLint, TypeScript, `git diff --check`, the 77-route webpack production build, and structured-data verification pass. Merge/publish remain human gates.
- **August 29 performance and accessibility pass (review ready; not published):** Changed the three self-hosted `next/font` faces to `display: optional`, moved Google Analytics to `lazyOnload`, removed the below-fold homepage reel poster's eager load, reduced hero/logo/client-image quality and responsive size hints, and corrected the audited contrast and link-name failures without changing SEO copy or conversion paths. ESLint, TypeScript, `git diff --check`, and the 77-route webpack production build pass. Local mobile Lighthouse: homepage 93 performance / 100 accessibility / 3.13 s LCP; Events 92-94 performance / 100 accessibility / 3.08-3.32 s LCP; both have CLS 0, TBT under 12 ms, no Vimeo player request, and no Google Tag Manager request in the critical measurement window. Local SEO 69 and best-practices 96 are expected because preview/localhost is intentionally noindexed and Vercel instrumentation endpoints are absent. A clean rebuild confirmed that the earlier 14 structured-data failures were stale `.next` output: all seven Media Bar Answers pages now emit Article schema without VideoObject, and video sitemap markup is limited to dedicated watch pages. Merge/publish remain human gates.
- **August 26 SEO audit Week 1 bundle (live via PR #10):** Aligned `/video-production/events` title, description, H1, hero copy, and proof with its conference and corporate-event search demand. Converted the homepage from a full-page Client Component to a Server Component with two small client islands for reveal behavior and the delayed Vimeo hero; mobile, reduced-motion, and data-saver visitors do not request the Vimeo player. Next.js intentionally serializes the root canonical URL without a trailing slash, so no duplicate custom canonical was added. ESLint, TypeScript, structured-data verification (12 service pages, 15 blog posts, 13 watch pages), `git diff --check`, and the webpack production build passed. PR #10 merged as `59ce95e`; Vercel production deployment `dpl_8xRCUGpBnUWAm9dSw9bvhrD1Cu4U` completed successfully, and live homepage/Events HTTP, metadata, canonical, H1, robots, sitemap, redirect, and mobile Vimeo-player checks passed. Post-deploy live Lighthouse reported SEO 100 on both pages; GSC performance monitoring remains a human follow-up.
- **August 19 2025 demo reel watch page:** Added WorkProject `1077104073` as `/work/watch/2025-demo-reel` so the homepage/showreel Vimeo is no longer marked up only on listing pages. VideoObject stays on the dedicated watch page only (`url` + `mainEntityOfPage` self). Homepage reel card and `/work` showreel now link to the watch URL; WorkGallery and sitemap video extras pick it up automatically. Do not put VideoObject back on `/`, `/work`, or service pages. GSC URL Inspection remains a manual gate.
- **August 19 GSC crawl-error 301s:** Hardened legacy ASP.NET `/Job/:id/:slug*` and `/Jobs` prefixes to explicit 301s toward `/careers` (covers the truncated GSC 5xx URL `/Job/5/professional-video-editor-contract---san-anto`, trailing slash, and `/jobs` nested paths that previously 404'd). Hardened `/News/Get/30` (truncated GSC duplicate `/News/Get/30/national-television-video-production-san-anto`) to `/video-production/commercials`, kept existing per-ID `/News/Get/{id}` mappings, and added `/News/Get/:id/:rest*` catch-all to `/blog`. Apex host rules target `https://www.mediabarproductions.com` + canonical path. Did not rebuild old CMS pages and did not invent a route for stale `/_next/static/chunks/*` 404s. Manual GSC URL Inspection remains a human gate.
- **August 12 service-page CTR copy:** Rewrote titles, metas, H1s, hero decks, selected H2s, and FAQs on `/video-production/events`, `/live-streaming`, `/post-production`, `/corporate`, and `/commercials` using GSC Web performance (last 3 months). Events copy now leads with event/conference video production. Live-streaming title is `San Antonio Live Streaming Company | Webcasts | Media Bar` (page-1 query at pos 9.9 with 0 clicks) and visible copy covers webcasts/hybrid events. Hardened legacy `/events`, `/commericals`, `/live-streaming-webcasting-san-antonio`, and `/video-post-production` to explicit 301s (plus trailing-slash and case variants). No invented claims. Brand/homepage left alone.
- **August 12 GSC video indexing fix:** Ground truth from Search Console (sc-domain:mediabarproductions.com, last update 8/9/26). Removed VideoObject JSON-LD from the 10 modern failing URLs (homepage demo reel 1077104073, `/work` 1180526566, RBFCU case study YouTube hpHKrVXhWnY, corporate 1193317757, interview 298919239, events 946447253, medical 1180540188, real-estate 1138383811, food 298056449, Media Bar Answers library) plus the 2 "not processed" service pages (commercials 1138375371, motion-graphics 1180540640). Players stay. Watch-page VideoObject now has `url`/`mainEntityOfPage`. Those two unprocessed films already have watch pages (`/work/watch/rbfcu-coyote-commercial`, `/work/watch/fleer-brilliants-superman`). Legacy `/aerial-video-photography` and `/News/Get/31/*` already 301/308 to `/video-production/aerial` and `/video-production/medical`; apex redirects to www. Sitemap video extras are watch URLs only. Do not request GSC indexing from this change (manual gate).
- **August 12 SEO quick wins (this session):** hardened `/sitemap.xml` so generation never throws (Invalid `Date` `toISOString()` in Next.js sitemap serialization was the 500 risk; blog markdown parsing was also removed from the sitemap path). Split H1 accents now include a real space so extracted heading text reads naturally. Homepage visible Q&As emit matching `FAQPage` JSON-LD. `/work` Vimeo posters and `/blog` listing thumbnails use title-derived alt text. Title tags only: `/locations/san-antonio` is now `San Antonio Video Production Company | Crew & Studio | Media Bar`; `/contact` is now `Contact Media Bar Productions | San Antonio Video Company`.
