...
- **August 12 GSC video indexing fix:** Google Search Console reported portfolio videos as "Video isn't on a watch page" because `/work` emitted a ~34-node VideoObject graph on a gallery listing, and homepage/service/FAQ/case-study pages also marked up sample players. VideoObject JSON-LD now emits only on dedicated single-video watch pages (`/work/watch/[slug]` and `/resources/media-bar-answers/[slug]`), with `url` and `mainEntityOfPage` pointing at that watch URL. Visible players remain on listing and service pages. Sitemap video extras stay on those watch URLs only and now include duration. Incomplete listing-page VideoObjects (CollectionPage items and RBFCU `Article.video` nodes missing duration) were removed; those were the likely "Video not processed" sources. Do not request GSC indexing from this change (manual gate).
